...
export function generateAvailabilitySnapshot(
  ...
  const lines: string[] = [
    ...
    "2. REGLA ESTRICTA PIANO VS. PIANO INFANTIL: La división es por RANGO DE EDAD Y EXPERIENCIA. Prof. Nathaly (Sala C) atiende exclusivamente Piano Infantil (niños pequeños / iniciales) y Canto. Prof. Fernando (Sala B) atiende Piano estándar, juvenil, adultos, niveles avanzados/Master y Violín. JAMÁS transferir a un alumno de Fernando con Nathaly por falta de cupo (quedaríamos pésimo ante el alumno). Si Fernando está lleno, ofrecer otro horario con Fernando o evaluar a futuro.",
